...
public class AddEffectsEditExEvent  extends JWizardDialog {
	...
	// Stringa per rappresentare l'espressione con cui si valuta l'effetto
	public String expr;
	// Stringa per rappresentare l'effetto (in toto)
	public String effect_string;
	
	public Vector<Vector<String>> att_arguments = H_EditExogenousEvent.attual_argum;  
	
	public AddEffectsEditExEvent(){
		
		setModal(true);
		
		setTitle("Wizard for add effects to an Exogenous Event");
	...
	public class Step1 extends JWizardPanel {
			...
			protected void makingVisible(){
				...
				if(n_argAT == 1){
					...
					objects_1 = XMLParser.getDataObjectsOf(vec.get(0).get(0),false);
					...
					objects_2 = XMLParser.getDataObjectsOf(vec.get(0).get(1),false);
					...
					objects_3 = XMLParser.getDataObjectsOf(vec.get(0).get(2),false);
					...
					objects_4 = XMLParser.getDataObjectsOf(vec.get(0).get(3),false);
					...
					objects_5 = XMLParser.getDataObjectsOf(vec.get(0).get(4),false);
	...
	public class Step3 extends JWizardPanel {
		...
		protected void makingVisible(){
			...
			expr = XMLParser.getDataObjectsOf(vec.get(1).get(0),false);
	...
	protected void finish(){
		effect_string = summary + expr;
		
		super.finish();
	}


} // CHIUDE LA CLASSE ESTERNA (l'outer class)
