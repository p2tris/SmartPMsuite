...
import com.mxgraph.smartml.control.H_EditExogenousEvent;
import com.mxgraph.smartml.model.XMLParser;


public class EditExogenousEvent extends JFrame 
{
	
	private static final long serialVersionUID = 1L;
	
	public H_EditExogenousEvent _handler = null;
	...
	public Vector<Vector<String>> vec_parIn; // per memorizzare gli argomenti in ingresso all'evento esogeno e i relativi tipi
	public Vector<String> vec_effects; // per memorizzare i dattagli dell'evento esogeno, nello specifico gli effetti (che sono tutti automatic)
	public String old_name; // per memorizzare il vecchio nome del task
	
	
	public EditExogenousEvent(String name_ev) {
		super("UPDATE EXOGENOUS EVENT");
		initComponent(name_ev);
		initHandler();
		
	}

	private void initHandler() {
		_handler = new H_EditExogenousEvent(this); 
	...
	private void initComponent(String n) {
		...
		name_textField = new JTextField(n); // CARICA IL NOME DELL'EVENTO ESOGENO
		...
		// parte di precompilazione dei campi
		old_name = n;
		...
		vec_parIn = XMLParser.getInputParametersOfExogenousEvent(n);
		n_ArgEvent = vec_parIn.size();
		...
		if(n_ArgEvent == 1){
			...
			objects_1 = XMLParser.getRelevantTypesForArgumentsOfExogenousEvents();
			...
			objects_2 = XMLParser.getRelevantTypesForArgumentsOfExogenousEvents();
			...
			objects_3 = XMLParser.getRelevantTypesForArgumentsOfExogenousEvents();
			...
			objects_4 = XMLParser.getRelevantTypesForArgumentsOfExogenousEvents();
			...
			objects_5 = XMLParser.getRelevantTypesForArgumentsOfExogenousEvents();
		...
		vec_effects = XMLParser.getExogenousEventEffects(n);
		// EFFETTI
		String effects_area = "";
		String app_eff = null;
		if(vec_effects.size() == 1){ // ho un solo effetto
			app_eff = vec_effects.firstElement();
			if(app_eff.contains("&gt;")){
			...
			effects_area = app_eff;
			eff_textArea.setText(effects_area);
		}
		else{ // ho pi� di un effetto
			for(int j=0;j<vec_effects.size();j++){
				app_eff = vec_effects.get(j);
				if(app_eff.contains("&gt;")){
					app_eff = app_eff.replace("&gt;", ">");
				}
				if(app_eff.contains("&lt;")){
				...
				if(j==vec_effects.size()-1)
					effects_area = effects_area + app_eff;
				
				else
					effects_area = effects_area + app_eff + " AND\n";
	...
	public Vector<Vector<String>> ArgsExEvent(){
