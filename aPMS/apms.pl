%%%%%%%%%%%%%%%%%%%% 
% FILE: aPMS.pl (24-11-2014)
% DESCRIPTION : The latest version of SmartPM, the Indigolog adaptive Process Management System
% VERSION : 0.2
% FIXED BUGS : Added a further interrupt **interrupt(and(neg(finished),exogenous), wait)** to wait the completion of a recovery procedure under execution.
% 	       Workitems work with multiple inputs/outputs - Exogenous events are correctly supported
...
initially(batterylevel(rb1),10).
initially(batterylevel(rb2),10).
...
interrupt(and(neg(finished),exogenous), wait),
...
?(writeln(Stream,('(provides act4 extinguisher)'))),	
