...
capability(B) :- domain(B,[hatchet,camera,gprs,extinguisher,digger,powerpack]).
...
/******************/
/* FORCE TO START */
prim_action(forceToStart(SRVC,ID,TASK)) :- id(ID),service(SRVC), task(TASK).
poss(forceToStart(SRVC,ID,TASK),true) :- id(ID),service(SRVC), task(TASK).
causes_val(forceToStart(SRVC,ID,TASK),reserved(SRVC,ID,TASK),true,true). 

prim_action(abortTask(SRVC,ID,TASK,_V)) :- task(TASK), service(SRVC), id(ID).
poss(abortTask(SRVC,ID,TASK,_V),true) :- id(ID),service(SRVC), task(TASK).
causes_val(abortTask(SRVC,ID,TASK,_V),reserved(SRVC,ID,TASK),false,true). 

fun_fluent(forced(ID,TASK)) :- id(ID), task(TASK).
causes_val(forceToStart(_,ID,TASK),forced(ID,TASK),true,true). 
causes_val(abortTask(_,ID,TASK,_),forced(ID,TASK),false,true).
/******************/
...
/*FLUENT FOR DESCRIBING TASKS' EXPECTED OUTPUTS*/

fun_fluent(exOut(ID,TASK)) :- id(ID), task(TASK).
causes_val(assign(SRVC,[workitem(TASK,ID,_INPUT,EXOUTPUT)]),exOut(ID,TASK),EXOUTPUT,true) :- service(SRVC), task(TASK), id(ID).

/*FLUENT FOR DESCRIBING IF A TASK IS ACTIVE */

fun_fluent(active(ID,TASK)) :- id(ID), task(TASK).
causes_val(assign(SRVC,[workitem(TASK,ID,_INPUT,_EXOUTPUT)]),active(ID,TASK),true,true) :- service(SRVC), task(TASK), id(ID).
causes_val(release(SRVC,[workitem(TASK,ID,_INPUT,_EXOUTPUT)]),active(ID,TASK),false,true) :- service(SRVC), task(TASK), id(ID).

/*FLUENT FOR DESCRIBING IF A TASK HAS BEEN COMPLETED */

fun_fluent(completed(ID,TASK)) :- id(ID), task(TASK).
causes_val(release(SRVC,[workitem(TASK,ID,_INPUT,_EXOUTPUT)]),completed(ID,TASK),true,true) :- service(SRVC), task(TASK), id(ID).
...
/*********************************************************/
causes_val(abortTask(SRVC,ID,go,[OUTPUT]),at_exp(SRVC),TO,and(neg(adapting),assigned(SRVC,[workitem(go,ID,[FROM,TO],[TO])]))) :- 
	actor(SRVC),id(ID),location_type(OUTPUT),location_type(FROM),location_type(TO).
/*********************************************************/
...
/*********************************************************/
causes_val(abortTask(SRVC,ID,evacuate,[OUTPUT]),evacuated_exp(LOC),true,
   and(neg(adapting),assigned(SRVC,[workitem(evacuate,ID,[LOC],[true])]))) :-
        service(SRVC),id(ID),location_type(LOC),boolean_type(true),boolean_type(OUTPUT).
/*********************************************************/
...
/*********************************************************/
causes_val(abortTask(SRVC,ID,extinguishfire,[OUTPUT]),status_exp(LOC),ok,
    and(assigned(SRVC,[workitem(extinguishfire,ID,[LOC],[ok])]),neg(adapting))) :-
        location_type(LOC),status_type(ok),status_type(OUTPUT).

causes_val(abortTask(SRVC,ID,removedebris,[OUTPUT]),status_exp(LOC),ok,
    and(assigned(SRVC,[workitem(removedebris,ID,[LOC],[ok])]),neg(adapting))) :-
        location_type(LOC),status_type(ok),status_type(OUTPUT).

causes_val(abortTask(SRVC,ID,updatestatus,[OUTPUT]),status_exp(LOC),ok,
    and(assigned(SRVC,[workitem(updatestatus,ID,[LOC],[ok])]),neg(adapting))) :-
	location_type(LOC),status_type(ok),status_type(OUTPUT).
/*********************************************************/
...
/*********************************************************/
causes_val(abortTask(SRVC,ID,takephoto,[OUTPUT]),photoTaken_exp(LOC),true,
  	and(neg(adapting),assigned(SRVC,[workitem(takephoto,ID,[LOC],[true])]))) :-
        	service(SRVC),id(ID),location_type(LOC),boolean_type(true),boolean_type(OUTPUT).
/*********************************************************/
...
/**********************/
initially(completed(ID,TASK),false) :- task(TASK), id(ID).
initially(active(ID,TASK),false) :- task(TASK), id(ID).
initially(forced(ID,TASK),false) :- task(TASK), id(ID).
/**********************/
...
/*******************************/
prim_action(resetPreExog). 
poss(resetPreExog,true). 
causes_val(resetPreExog,pre_exogenous,false,true). 
rel_fluent(pre_exogenous). 
initially(pre_exogenous,false). 
/*******************************/
...
/**********************************/
proc(applySideEffects,[ndet([?(and(active(id_4,go),neg(reserved(act2,id_4,go)))),forceToStart(act2,id_4,go)],
			    [?(and(active(id_4,go),reserved(act2,id_4,go))),abortTask(act2,id_4,go,exOut(id_4,go))]),
		            resetPreExog]).
/**********************************/
...
proc(executionHelp(N,[workitem(T,D,I,E)|TAIL]),[start(N,D,T,I,E), if(forced(D,T),abortTask(N,D,T,exOut(D,T)),ackCompl(N,D,T)), executionHelp(N,TAIL)]). 
...
interrupt(and(neg(finished),pre_exogenous), applySideEffects),
...
proc(printALL,[?(writeln('****FREE****')),
	       ...
	       ?(write('isConnected(act1) = ')),if(isConnected(act1),?(writeln(true)),?(writeln(false))),
 	       ...
 	       ?(write('isConnected(act4) = ')),if(isConnected(act4),?(writeln(true)),?(writeln(false))),
               ?(writeln('****ACTIVE - COMPLETED****')),
	       ?(write('active(id_1,go) = ')),?(writeln(active(id_1,go))),
	       ?(write('ExOut(id_1,go) = ')),if(or(active(id_1,go),completed(id_1,go)),?(writeln(exOut(id_1,go))),?(writeln('NO EXOUT'))),
               ?(write('active(id_2,takephoto) = ')),?(writeln(active(id_2,takephoto))),
	       ?(write('ExOut(id_2,takephoto) = ')),if(or(active(id_2,takephoto),completed(id_2,takephoto)),?(writeln(exOut(id_2,takephoto))),?(writeln('NO EXOUT'))),
               ?(write('active(id_3,updatestatus) = ')),?(writeln(active(id_3,updatestatus))),
	       ?(write('ExOut(id_3,updatestatus) = ')),if(or(active(id_3,updatestatus),completed(id_3,updatestatus)),?(writeln(exOut(id_3,updatestatus))),?(writeln('NO EXOUT'))),
               ?(write('active(id_4,go) = ')),?(writeln(active(id_4,go))),
	       ?(write('ExOut(id_4,go) = ')),if(or(active(id_4,go),completed(id_4,go)),?(writeln(exOut(id_4,go))),?(writeln('NO EXOUT'))),
	       ?(write('active(id_5,evacuate) = ')),?(writeln(active(id_5,evacuate))),
	       ?(write('ExOut(id_5,evacuate) = ')),if(or(active(id_5,evacuate),completed(id_5,evacuate)),?(writeln(exOut(id_5,evacuate))),?(writeln('NO EXOUT'))),
               ?(write('active(id_6,updatestatus) = ')),?(writeln(active(id_6,updatestatus))),
	       ?(write('ExOut(id_6,updatestatus) = ')),if(or(active(id_6,updatestatus),completed(id_6,updatestatus)),?(writeln(exOut(id_6,updatestatus))),?(writeln('NO EXOUT'))),
               ?(write('active(id_7,go) = ')),?(writeln(active(id_7,go))),
	       ?(write('ExOut(id_7,go) = ')),if(or(active(id_7,go),completed(id_7,go)),?(writeln(exOut(id_7,go))),?(writeln('NO EXOUT'))),
               ?(write('active(id_8,evacuate) = ')),?(writeln(active(id_8,evacuate))),
	       ?(write('ExOut(id_8,evacuate) = ')),if(or(active(id_8,evacuate),completed(id_8,evacuate)),?(writeln(exOut(id_8,evacuate))),?(writeln('NO EXOUT'))),
               ?(write('active(id_9,updatestatus) = ')),?(writeln(active(id_9,updatestatus))),
	       ?(write('ExOut(id_9,updatestatus) = ')),if(or(active(id_9,updatestatus),completed(id_9,updatestatus)),?(writeln(exOut(id_9,updatestatus))),?(writeln('NO EXOUT'))),
	       ?(write('completed(id_1,go) = ')),?(writeln(completed(id_1,go))),
               ?(write('completed(id_2,takephoto) = ')),?(writeln(completed(id_2,takephoto))),
               ?(write('completed(id_3,updatestatus) = ')),?(writeln(completed(id_3,updatestatus))),
               ?(write('completed(id_4,go) = ')),?(writeln(completed(id_4,go))),
	       ?(write('completed(id_5,evacuate) = ')),?(writeln(completed(id_5,evacuate))),
               ?(write('completed(id_6,updatestatus) = ')),?(writeln(completed(id_6,updatestatus))),
               ?(write('completed(id_7,go) = ')),?(writeln(completed(id_7,go))),
               ?(write('completed(id_8,evacuate) = ')),?(writeln(completed(id_8,evacuate))),
               ?(write('completed(id_9,updatestatus) = ')),?(writeln(completed(id_9,updatestatus)))
]).
